...
import { Student, RESEARCH_AREAS, SKILLS } from '../../types';
...
const sections = [
  ...
  },
  {
    ...
    fields: ['university', 'major', 'year', 'gpa']
  },
  {
    id: 'research',
    title: 'Research Interests',
    subtitle: 'What excites you about research?',
    fields: ['researchInterests', 'skills']
  },
  {
    id: 'experience',
    title: 'Experience & Goals',
    subtitle: 'Your background and aspirations',
    fields: ['experience', 'careerGoals', 'availability']
  }
];
...
const StudentOnboarding: React.FC = () => {
  ...
  const validateSection = (sectionIndex: number): boolean => {
    ...
    section.fields.forEach(field => {
      ...
      if (field === 'gpa' && value && (value < 0 || value > 4)) {
        newErrors[field] = 'GPA must be between 0.0 and 4.0';
        isValid = false;
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const studentData: Student = {
        ...
        researchInterests: answers.researchInterests || [],
      ...
      if (typeof window !== 'undefined') {
        const existingStudents = JSON.parse(localStorage.getItem('labyrinth_students') || '[]');
        existingStudents.push(studentData);
        localStorage.setItem('labyrinth_students', JSON.stringify(existingStudents));
      }
      
      router.push(`/student/matches?id=${studentData.id}`);
    } catch (error) {
  ...
  const renderField = (field: string, sectionIndex: number) => {
    ...
    switch (field) {
              ...
              {['He/Him/His', 'She/Her/Hers', 'They/Them/Theirs', 'Other'].map((option) => (
                <motion.label
                  ...
                  className={`flex items-center p-3 rounded-lg border-2 cursor-pointer transition-all duration-200 ${
                    value === option 
                      ? 'border-uiuc-orange bg-orange-50' 
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded-full border-2 mr-3 ${
                    value === option ? 'border-uiuc-orange bg-uiuc-orange' : 'border-gray-400'
                  }`}>
      ...
      case 'major':
        return (
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-800">Major *</label>
            <input
              type="text"
              value={value || ''}
              onChange={(e) => handleInputChange(field, e.target.value)}
      ...
      case 'year':
        return (
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-gray-800">Academic Year *</label>
            <div className="grid grid-cols-2 gap-3">
      ...
      case 'gpa':
        return (
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-800">GPA (0.0 - 4.0) *</label>
            <input
              type="number"
              min="0"
              max="4"
      ...
      case 'researchInterests':
        return (
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-gray-800">Research Interests * (Select all that apply)</label>
              ...
              {RESEARCH_AREAS.map((area) => (
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded border-2 mr-2 flex items-center justify-center ${
                    (value || []).includes(area.name) ? 'border-uiuc-orange bg-uiuc-orange' : 'border-gray-400'
                  }`}>
                    {(value || []).includes(area.name) && (
      ...
      case 'skills':
        return (
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-gray-800">Skills * (Select all that apply)</label>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-64 overflow-y-auto border border-gray-300 rounded-lg p-4">
              {SKILLS.map((skill) => (
                <motion.label
                  key={skill}
                ...
                >
                  ...
                  />
                  <div className={`w-4 h-4 rounded border-2 mr-2 flex items-center justify-center ${
                    (value || []).includes(skill) ? 'border-uiuc-orange bg-uiuc-orange' : 'border-gray-400'
                  }`}>
                    {(value || []).includes(skill) && (
                      <CheckIcon className="w-3 h-3 text-white" />
                    )}
      ...
      case 'experience':
        return (
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-800">Relevant Experience *</label>
            <textarea
              rows={4}
              value={value || ''}
              onChange={(e) => handleInputChange(field, e.target.value)}
      ...
      case 'careerGoals':
        return (
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-800">Career Goals *</label>
            <textarea
              rows={4}
              value={value || ''}
              onChange={(e) => handleInputChange(field, e.target.value)}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-gray-600 text-sm">Student Registration</span>
            </div>
      ...
      <div className="fixed top-20 right-6 z-40">
        ...
        >
          <div className="text-center mb-2">
            <div className="text-2xl font-bold text-uiuc-orange">{Math.round(progress)}%</div>
            <div className="text-xs text-gray-600">Complete</div>
          </div>
          <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-uiuc-orange"
              initial={{ width: 0 }}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 hover:shadow-xl transition-all duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {section.fields.map((field) => (
                    <motion.div
                      ...
                      className={field === 'ethnicity' || field === 'researchInterests' || field === 'skills' || field === 'experience' || field === 'careerGoals' || field === 'year' || field === 'availability' ? 'md:col-span-2' : ''}
                    >
                ...
                <div className="flex justify-center mt-8">
                  <motion.button
                    ...
                    className="bg-uiuc-orange text-white px-8 py-3 rounded-lg font-semibold shadow-lg hover:bg-uiuc-orange-dark transition-all duration-300"
                  >
        ...
        >
          <div className="bg-gray-50 rounded-2xl p-8 text-center border border-gray-200">
            <h3 className="text-2xl font-bold text-gray-900 mb-4">Ready to Find Your Matches?</h3>
            <p className="text-gray-600 mb-8">
              Complete your profile to discover professors whose research aligns with your interests
            </p>
            ...
            <motion.button
              ...
              className="bg-uiuc-blue text-white px-12 py-4 rounded-lg font-bold text-lg shadow-lg hover:bg-uiuc-blue-dark transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
